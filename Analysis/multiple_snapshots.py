...
# To search dataset for a specific combination of Signal Type + Violation condition: 'compliant', 'bw', 'dc', 'oob', 'erp'
import pickle
import numpy as np
...
def find_specific_example(metadata, snapshots, label_filter, condition):
    for i, meta in enumerate(metadata):
        ...
        is_bw = rogue.get('bw_violation', False)
        is_oob = rogue.get('oob_violation', False)
        is_erp = rogue.get('erp_violation', False)
...
def plot_spectrogram(ax, sig, fs, title, caption_text):
    ...
    ax.set_ylim(-500, 500) # View window +/- 500 kHz
    ax.grid(alpha=0.2)
...
def run_visualization():
    ...
    # Save Output
    save_name = "full_violation_matrix0db.png"
